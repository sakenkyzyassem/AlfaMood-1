...
export default class HistoryPage extends Component {
  ...
  getData=async()=>{
    try
    {
      await this.postMethod('getMood',this.state.server,{user_id:this.state.user_id,timeZone:this.state.timeZone});
    }
    catch(e)
    {
      this.setState({loading:false});
      alert("Что то пошло не так. Данные не загрузились .Проверьте подключение к интернету");
    }
  }
      ...
      <View style={[styles.MainContainer,{backgroundColor:'white'}]}>
        {
          ...
          <View style={styles.MainContainer}>
            ...
            {
              this.state.data==null ? <NoData/> :
              <BigHistoryView 
                data={this.state.data} 
                handlePress={
                  (day)=>this.setTodayDate(day)
                }/>
            }
...
BigHistoryView = (props) => {
  ...
  for(var i = 0;i<3;i++){
    ...
    if (array !== undefined && array.length != 0) 
    {
      data.push( <HistoryView 
        data={array} 
        handlePress={(day)=>props.handlePress(day)} 
        day={i} 
        key={i}
      />)
    }
    ...
    <View style={styles.HistoryViewContainer}>
      <View style={styles.HistoryViewText}>
        <Image 
          ...
          style={styles.HistoryViewCalendar}/>
        <Text>
          {a[0].date}
        </Text>
      </View>
      ...
      <TouchableOpacity 
        style={styles.HistoryViewImageContainer} 
        onPress={()=>props.handlePress(props.day)}>
          {data}
      </TouchableOpacity>
    </View>
  )
